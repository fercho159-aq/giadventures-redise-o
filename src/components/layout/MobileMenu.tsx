...
import { NAV_LINKS, INSTAGRAM_URL, TIKTOK_URL, WHATSAPP_LINK } from "@/lib/constants";
    ...
    <AnimatePresence>
      {isOpen && (
        ...
        >
          ...
          {/* Menu panel */}
          <motion.nav
            className="absolute right-0 top-0 bottom-0 w-80 max-w-[85vw] bg-slate-900 flex flex-col"
            ...
            <div className="flex-1 px-6 space-y-2">
              {NAV_LINKS.map((link, i) => (
                ...
                >
                  <Link
                    href={link.href}
                    onClick={onClose}
                    className="block rounded-lg px-4 py-3 text-xl font-heading font-semibold text-white hover:bg-slate-800 transition-colors"
                  >
                    {t(link.labelKey)}
                  </Link>
                </motion.div>
              ...
              >
                <Link
                  href="/expediciones"
                  onClick={onClose}
                  className="mt-4 block rounded-xl bg-summit-500 px-4 py-3 text-center text-lg font-heading font-bold text-white hover:bg-summit-600 transition-colors"
                >
                  {t("book")}
                </Link>
              </motion.div>
            ...
            <div className="border-t border-slate-700 p-6 space-y-4">
              ...
              <div className="flex items-center justify-center gap-4">
                ...
                <a
                  ...
                  rel="noopener noreferrer"
                  aria-label="WhatsApp"
                  className="rounded-full p-2 text-slate-300 hover:bg-slate-800 hover:text-[#25D366] transition-colors"
                >
                  <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
